CREATE FUNCTION [dbo].[func_pt_itemOfficial_NonRef]
(
	@storeId AS bigint,
	@statusId AS int,
	@cityId AS int
...
(
	SELECT DISTINCT 
                         I.id, 
						ISNULL(I.title, '') + ' - ' + ISNULL(store.title, '')  AS t,
                         ...
                         + '''' + ')' + '"' + 'style=' + '"' + 'cursor:pointer;' + '"' + '/><span style=' + '"' + 'position:absolute; background-color: #3c8dbc;color: #ffffff;padding: 2px 9px 2px 9px;border-radius: 30px; margin-right: -10px;' + '"' + '>' + CAST((imgCount.countOfImg)
						  ...
						  IG.title AS grp, ISNULL(U.fname, '') + ' ' + ISNULL(U.lname, '') AS n,
						 ...
						 ISNULL(store.title, '') AS st,
						 ISNULL(city.title, '') AS ct, 
                         I.fk_status_id, 
						 itemQty.pk_fk_store_id AS fk_store_id,
						 store.fk_city_id, 
						 store.title AS sto_title,
