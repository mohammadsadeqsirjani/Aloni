CREATE PROCEDURE [dbo].[SP_addNewOrderDoc_Depricated_2]
...
BEGIN
	SET @rMsg = dbo.func_getSysMsg('docUsedAsReference', OBJECT_NAME(@@PROCID), @ClientLanguage, 'the target document is used as Reference to another doc.');--وضعیت سفارش / سبد خرید تغییر کرده است.
...
BEGIN TRY
	...
	@hdr_fk_status_id = fk_status_id
	FROM TB_ORDER_HDR
	WHERE id = @lastOrderHdrId;


	begin /* بررسی تکراری نبودن کالا در ردیف های متفاوت*/
	IF (
			EXISTS (
	...
	begin /*ثبت نهایی سفارش*/

	IF (@docType = 1) 
	BEGIN
		--special validations
		--begin /*بررسی وضعیت فعلی سفارش*/
		--if(@hdr_fk_status_id <> 23)
		--begin
		--SET @rMsg = dbo.func_getSysMsg('unAuthorized', OBJECT_NAME(@@PROCID), @ClientLanguage, 'the current status of order is not unAuthorized for this operation.');
		--GOTO fail;
		--end
		--end;		
	...
	begin /*بروز رسانی (تکمیل) سربرگ سفارش*/
		UPDATE TB_ORDER_HDR
		SET fk_status_id = 24
