CREATE PROCEDURE [dbo].[SP_addFavoriteItem]
...
AS
	...
	if(exists(select top 1 1 from TB_STORE_ITEM_FAVORITE where pk_fk_item_id = @itemId and pk_fk_store_id = @storeId and pk_fk_usr_id = @userId))
	begin
		set @rMsg = dbo.func_getSysMsg('item exists',OBJECT_NAME(@@PROCID),@clientLanguage,'این آیتم از قبل در بین آیتم های مورد علاقه شما در پنل موجود می باشد'); 
		set @rCode = 0
		return
	end
	begin try
		
			insert TB_STORE_ITEM_FAVORITE(pk_fk_store_id,pk_fk_item_id,pk_fk_usr_id) values (@storeId,@itemId,@userId)
			set @rCode = 1
