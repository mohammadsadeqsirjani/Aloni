...
-- =============================================
CREATE PROCEDURE [dbo].[SP_PT_getTechnicalInfoListByPageId_Child]
	@clientLanguage as char(2) = 'fa',
	@appId as tinyint = null,
	@clientIp as varchar(50) = null,
	@pageId as smallint,
	@pageId_child as smallint
AS
BEGIN
	SET NOCOUNT ON;
...
ISNULL(TITB.id , ISNULL(TIT_fkRefTbl.id , '') ) table_id,
ISNULL(TIT_fkRef.title , '' ) fk_ref_title ,
ISNULL(TI.[order] , TIT_fkRefR.[order]) as [order] ,
ISNULL(TI.type , TIT_fkRefR.type)  type_id ,
case
when EXISTS(select * from TB_TYP_TECHNICALINFO CT where CT.fk_ref = TI.id and CT.fk_technicalinfo_page_id = @pageId_child) 
then 1
else 0
end as ignored
